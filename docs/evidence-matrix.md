...
| 5. Assertions for invariants | `assert` is used to defend internal ring-buffer validity assumptions; ACSL predicates document structural and FIFO invariants | Indirectly supported by all behavioral tests; optional `make verify` and `make verify-fifo` check contracts with Frama-C/WP | `examples/rule05_assertions_good.c`, `examples/rule05_assertions_bad.c` | Assertions complement runtime error handling; contracts complement both tests and assertions |
...
| 10. Warning-clean, tool-friendly code | `Makefile` enables strict warning flags, optional Frama-C targets, and optional `cppcheck`; CI runs build/test/examples | CI checks the same commands expected of a reviewer; `make verify` and `make verify-fifo` can be run where Frama-C is installed | `examples/rule10_warnings_good.c`, `examples/rule10_warnings_bad.c` | Static analysis is documented, with room for future tooling expansion |
...
5. Read `docs/frama-c-contracts.md`
6. Check `docs/verification-matrix.md` and `docs/safety-case.md`
7. Check `docs/reviewer-readiness.md`
8. Use `docs/review-results.md` to record reviewer command outcomes
6. Finish with `docs/reflection.md`
